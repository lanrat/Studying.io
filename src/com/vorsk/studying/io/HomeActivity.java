...
public class HomeActivity extends SherlockActivity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		new AppRate(this).setShowIfAppHasCrashed(false)
				.setMinDaysUntilPrompt(4).setMinLaunchesUntilPrompt(10).init();
	}
	...
	public void onClick(View v) {
		...
		case R.id.hintButton:
			if (question != null && hintCount >= question.getHintCount()) {
				Toast.makeText(this, getString(R.string.no_more_hints),
	  ...
	  private void scrollDown(){
	        ...
	        new Thread(new Runnable() {
	            @Override
	            public void run() {
	            	//sleep needed to let the animation run
	...
	private void updateDisplay() {
		...
		if (question != null) {
			((TextView) findViewById(R.id.questionText)).setText(question
					.getQuestion());
			((TextView) findViewById(R.id.exampleText)).setText(question
					.getExample());
			((TextView) findViewById(R.id.hintText)).setText(question
					.getHint(hintCount));
			((TextView) findViewById(R.id.answerText)).setText(question
					.getAnswer());
		}
	}
